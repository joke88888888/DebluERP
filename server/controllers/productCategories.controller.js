...
exports.update = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'UPDATE product_categories SET code=?, name=?, description=?, is_active=? WHERE id=?',
...
exports.remove = async (req, res) => {
  try {
    const [result] = await db.query('UPDATE product_categories SET is_active = 0 WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) return error(res, 'Not found', 404);
