...
exports.getAll = async (req, res) => {
  try {
    ...
    let sql = 'SELECT * FROM production_methods';
    const params = [];
...
exports.getById = async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM production_methods WHERE id = ?', [req.params.id]);
...
exports.create = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'INSERT INTO production_methods (code, name, description, is_active) VALUES (?, ?, ?, ?)',
      [code, name, description || null, is_active]
    );
    const [rows] = await db.query('SELECT * FROM production_methods WHERE id = ?', [result.insertId]);
...
exports.update = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'UPDATE production_methods SET code=?, name=?, description=?, is_active=? WHERE id=?',
...
exports.remove = async (req, res) => {
  try {
    const [result] = await db.query('UPDATE production_methods SET is_active = 0 WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) return error(res, 'Not found', 404);
