...
import { Add, Edit, Delete, Search, AddCircleOutline, RemoveCircleOutline, Layers } from '@mui/icons-material';
import ConfirmDialog from '../../components/common/ConfirmDialog';
import AppSnackbar from '../../components/common/Snackbar';
import useSnackbar from '../../hooks/useSnackbar';
import { lidMoldsApi, brandsApi, sizesApi } from '../../services';

const newSizeRow = () => ({ _id: Math.random(), size_id: '', quantity: 1 });

export default function LidMoldsPage() {
  ...
  const [form, setForm] = useState({ brand_id: '', name: '', is_active: true });
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await lidMoldsApi.getAll();
  ...
  const openCreate = () => {
    setEditItem(null);
    setForm({ brand_id: '', name: '', is_active: true });
  ...
  const openEdit = (item) => {
    setEditItem(item);
    setForm({ brand_id: item.brand_id, name: item.name, is_active: Boolean(item.is_active) });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editItem) { await lidMoldsApi.update(editItem.id, payload); showSnack('อัพเดตสำเร็จ'); }
      else { await lidMoldsApi.create(payload); showSnack('เพิ่มข้อมูลสำเร็จ'); }
      setDialogOpen(false);
      fetchData();
    } catch (err) {
      showSnack(err.response?.data?.message || 'เกิดข้อผิดพลาด', 'error');
  ...
  const handleDelete = async () => {
    try { await lidMoldsApi.remove(deleteId); showSnack('ลบข้อมูลสำเร็จ'); setDeleteId(null); fetchData(); }
    catch (err) { showSnack(err.response?.data?.message || 'เกิดข้อผิดพลาด', 'error'); }
  };

  const columns = [
    { field: 'brand_name', headerName: 'แบรนด์', width: 130 },
    { field: 'name', headerName: 'แบบฝาโมล์', flex: 1 },
    {
        ...
        return (
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, py: 0.5 }}>
            {arr.map((s, i) => {
              ...
              return <Chip key={i} size="small" label={`${sz?.size_value || s.size_id} × ${s.quantity}`} variant="outlined" color="primary" />;
            })}
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Box sx={{ p: 1, borderRadius: 2, bgcolor: 'primary.main', display: 'flex' }}>
            <Layers sx={{ color: 'white', fontSize: 28 }} />
          </Box>
          <Box>
            <Typography variant="h5" fontWeight={700}>ฝาโมล์</Typography>
            <Typography variant="body2" color="text.secondary">จัดการข้อมูลแบบฝาโมล์</Typography>
          </Box>
        </Box>
        <Button variant="contained" startIcon={<Add />} onClick={openCreate} sx={{ borderRadius: 2 }}>เพิ่มฝาโมล์</Button>
      </Box>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ pb: 1 }}>
          <Typography variant="h6" fontWeight={700}>{editItem ? 'แก้ไขฝาโมล์' : 'เพิ่มฝาโมล์'}</Typography>
        </DialogTitle>
        <Divider />
        <DialogContent>
          <Stack spacing={2} sx={{ pt: 2 }}>
            ...
            <TextField label="แบบฝาโมล์ (ไม่เกิน 20 ตัวอักษร) *" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
              inputProps={{ maxLength: 20 }} helperText={`${form.name.length}/20 ตัวอักษร`} fullWidth />
      ...
      <ConfirmDialog open={Boolean(deleteId)} title="ยืนยันการลบ" message="คุณต้องการลบฝาโมล์นี้ใช่หรือไม่?" onConfirm={handleDelete} onClose={() => setDeleteId(null)} />
